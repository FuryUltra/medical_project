...
const PatientForm = () => {
    ...
    const [afficherChamps, setAfficherChamps] = useState(false);

    const handleYesBiopsy = () => {
        setAfficherChamps(true);
    };

    const handleNoBiopsy = () => {
        setAfficherChamps(false);
        setValue('dateBiopsy', '');
    ...
    const onSubmit = (data) => {
        fetch("http://localhost:8000/colon_cancer", {
            ...
            .then((resp) => {
                ...
                reset();
                router.push(`/forms/colon_cancer/list_items`);
            })
            .catch((error) => {
        ...
        try {
            ...
            if (response.ok) {
                const updatedResponses = responses.map((response) =>
                    response.id === id ? { ...response, ...updatedData } : response
                );
